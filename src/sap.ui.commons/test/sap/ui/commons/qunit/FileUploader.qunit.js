...
sap.ui.define([
...
], function(Library, qutils, nextUIUpdate, FileUploader, library, StaticArea, TooltipBase, InvisibleText, Label, TextField, Device, jQuery) {
	"use strict";

	// shortcut for sap.ui.commons.FileUploaderHttpRequestMethod
	var FileUploaderHttpRequestMethod = library.FileUploaderHttpRequestMethod;
	...
	*/
	...
	QUnit.test("Test valueStateText property - setter/getter", async function (assert) {
		...
		// We are skipping this test because this functionality is not supported by TextField (in unified, an Input that supports it is used).
	...
	QUnit.test("AriaLabelledBy", async function(assert) {
		...
		var oFileUploader = new FileUploader("fu"),
			...
			aLabelledBy = [
				new TextField({text: "Labelled by 1"}),
				new TextField("labelledby2", {text: "Labelled by 2"}),
				new TextField({text: "Labelled by 3"})
			];
	...
	QUnit.test("AriaDescribedBy", function(assert) {
		...
		var oFileUploader = new FileUploader("fu"),
			...
			oBrowse = oFileUploader.oBrowse,
			aDescribedBy = [
				new TextField({text: "Described by 1"}),
				new TextField("describedby2", {text: "Described by 2"}),
				new TextField({text: "Described by 3"})
			];
	...
	QUnit.test("Label added dynamicaly", async function (assert) {
		...
		oNewLabel.placeAt("qunit-fixture");
		await nextUIUpdate();
	...
	QUnit.test("Description for default FileUploader", async function (assert) {
		// Setup
		var oFileUploader = new FileUploader("fu"),
			oRB = Library.getResourceBundleFor("sap.ui.commons");
	...
	QUnit.test("Description for required FileUploader", async function (assert) {
		...
		var oLabel = new Label({ text: "Label", labelFor: "fu", required: true }),
			oFileUploader = new FileUploader("fu"),
			sRequiredText = Library.getResourceBundleFor("sap.ui.commons").getText("FILEUPLOAD_REQUIRED");
	...
	QUnit.test("Internal hidden label for the Input", async function (assert) {
		var oFileUploader = new FileUploader(),
			sExpectedLabelId = InvisibleText.getStaticId("sap.ui.commons", "FILEUPLOAD_FILENAME"),
			aInputLabels;
