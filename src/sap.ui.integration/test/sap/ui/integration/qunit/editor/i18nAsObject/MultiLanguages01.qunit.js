			...
			return new Promise(function (resolve, reject) {
				...
				EditorQunitUtils.isFieldReady(that.oEditor).then(function () {
					...
					EditorQunitUtils.isReady(that.oEditor).then(async function () {
						...
						assert.equal(oField4.getAggregation("_field").getValue(), "String 4 English", "oField1: String 4 English");
						...
						await nextUIUpdate();
						EditorQunitUtils.wait(2000).then(async function () {
							...
							EditorQunitUtils.wait().then(async function () {
								...
								EditorQunitUtils.wait().then(function () {
									...
									assert.equal(aHeaderItems4[2].getItems()[1].getValue(), "String 4 English", "oTranslationPopover4 Header: String 4 English");
						...
						EditorQunitUtils.wait(2000).then(async function () {
							...
							EditorQunitUtils.wait().then(async function () {
								...
								for (var i = 1; i < oLanguageItems3.length; i++) {
									var sLanguage = oLanguageItems3[i].getCustomData()[0].getKey();
									var sCurrentValue = oLanguageItems3[i].getContent()[0].getItems()[1].getValue();
									assert.equal(sCurrentValue, "String 3", "oTranslationPopover3 Content: item " + i + " " + sLanguage + ", current: " + sCurrentValue + ", expected: ");
								}
								var oCancelButton3 = oTranslationPopover3.getFooter().getContent()[2];
								oCancelButton3.firePress();
