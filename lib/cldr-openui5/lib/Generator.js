...
const sLocalizationPath = join(__dirname, "../../..", "src/sap.ui.core/src/sap/base/i18n/Localization.js");
...
export default class Generator extends EventEmitter {
	...
	static aUI5Tags = ["ar", "ar_EG", "ar_SA", "bg", "ca", "cnr", "cs", "cy", "da", "de", "de_AT", "de_CH", "el",
		"el_CY", "en", "en_AU", "en_GB", "en_HK", "en_IE", "en_IN", "en_NZ", "en_PG", "en_SG", "en_ZA", "es",
		"es_AR", "es_BO", "es_CL", "es_CO", "es_MX", "es_PE", "es_UY", "es_VE", "et", "fa", "fi", "fr",
		"fr_BE", "fr_CA", "fr_CH", "fr_LU", "he", "hi", "hr", "hu", "id", "it", "it_CH", "ja", "kk", "ko", "lt",
		"lv", "mk", "ms", "nb", "nl", "nl_BE", "pl", "pt", "pt_PT", "ro", "ru", "ru_UA", "sk", "sl", "sr", "sr_Latn",
		"sv", "th", "tr", "uk", "vi", "zh_CN", "zh_HK", "zh_SG", "zh_TW"];
	static aRTLLocales = [];
	...
	generateLocaleFile(sUI5Tag) {
		...
		if (oResult.rtl) {
			Generator.aRTLLocales.push(sUI5Tag);
		}
		...
		this._oTimezones.updateLocaleTimezones(sCLDRTag, oResult);
		Generator.addStaticData(oResult, sCLDRTag);
		Generator.checkUnsupportedRTLCodes(sUI5Tag, oResult);
	...
	async start() {
		...
		Generator.aUI5Tags.forEach((sUI5Tag) => this.generateLocaleFile(sUI5Tag));
		...
		this._aTasks.push(async () => {
			...
			await Generator.writeLocaleIDsToFile(sLocaleDataPath, "A_SUPPORTED_LOCALES = ", Generator.aUI5Tags);
			await Generator.writeLocaleIDsToFile(sLocalizationPath, "A_RTL_LOCALES = ", Generator.aRTLLocales);
		});
	 ...
	 */
	static checkUnsupportedRTLCodes(sUI5Tag, oResult) {
	...
	static #oCurrencySignReplacements = {
		ar: {
			...
			transformTrailingCurrencyShort: true
		},
		fa: {
			currencySign: "\u202a¤\u202c",
			currencySignTrailing: "\u202a¤\u202c",
		...
		he: {
			...
			shortCurrencySignTrailing: "\u200e¤",
			transformTrailingCurrency: false,
			transformTrailingCurrencyShort: false
		},
		others: {
			currencySign: "¤",
			currencySignTrailing: "¤",
	...
	static updateCurrencyFormats(sCLDRTag, oResult) {
		...
		oResult["currencyFormat-sap-short"] = Generator.updateShortCurrencyFormats(oCurrencyFormatShort,
			oReplacements.shortCurrencySign, sCurrencySpacingBefore, oReplacements.transformTrailingCurrencyShort,
	...
	/**
	 * Writes the given array of locale IDs to the given file path at the position of the given array anchor.
	 * If the array exceeds the line length of 80 characters, the array is formatted with line breaks.
	 *
	 * @param {String} sFilePath The path to the file to write the locale IDs to
	 * @param {String} sArrayAnchor The anchor string to search for in the file referenced by the <coe>sFilePath</code>
	 * @param {Array<String>} aLocaleIds The array of locale IDs to write into the file
	 */
	static async writeLocaleIDsToFile(sFilePath, sArrayAnchor, aLocaleIds) {
		let sFileContent = await fs.promises.readFile(sFilePath, "utf8");
		const iArrayStart = sFileContent.indexOf(sArrayAnchor);
		const iArrayEnd = sFileContent.indexOf(";", iArrayStart);
		// Format the array with line breaks after 80 characters
		const aFormattedArray = JSON.stringify(aLocaleIds)
			.replace(/(.{80,}?,)/g, "$1\n\t\t");

		sFileContent = sFileContent.slice(0, iArrayStart) + sArrayAnchor + aFormattedArray
			+ sFileContent.slice(iArrayEnd);

		fs.promises.writeFile(sFilePath, sFileContent);
	}
}
