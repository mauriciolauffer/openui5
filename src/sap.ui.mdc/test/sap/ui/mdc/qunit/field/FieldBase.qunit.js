...
sap.ui.define([
	...
	const fnOnlyEQ = () => {return [OperatorName.EQ];};
	...
	QUnit.test("default delegate", (assert) => {
	...
	QUnit.test("call of delegate functions", async (assert) => {
		const oPayload = {x: 1};
		const oDelegate = sap.ui.require("sap/ui/mdc/field/FieldBaseDelegate");
	...
	function defaultRendering(assert) {
		...
		const oDialog = oValueHelp?.getDialog();
		assert.ok(oDialog, "Dialog used in ValueHelp");
		assert.ok(oDialog instanceof Dialog, "Dialog used");
		const aDialogContent = oDialog?.getContent()[0];
		assert.ok(aDialogContent instanceof Conditions, "ConditionPanel used");
	...
	QUnit.test("default rendering, async loading of control", async (assert) => {

		const oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/ui/mdc/field/FieldMultiInput").onFirstCall().returns(undefined);
		oStub.withArgs(["sap/ui/mdc/field/FieldMultiInput"]).onFirstCall().callsFake((vDependencies, fnCallback, fnErrCallback) => {
			setTimeout(() => {
		...
		const aContent = oField.getAggregation("_content");
		assert.notOk(aContent?.length > 0, "Content not created sync");

		const oDomRef = oField.getFocusDomRef();
		assert.equal(oDomRef?.id, "F1", "Field rendered without content -> Focus DomRef points to Field");
		assert.equal(oDomRef && jQuery(oDomRef).attr("tabIndex"), "-1", "DomRef is focusable");

		const fnDone = assert.async();
		setTimeout(() => { // to wait async creation of inner control
			setTimeout(async () => { // to wait for invalidation
				await nextUIUpdate;
				defaultRendering(assert);
	...
	QUnit.test("default rendering, async loading of delegate", async (assert) => {

		oField.destroy();
		const oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/ui/mdc/field/FieldBaseDelegate").onFirstCall().returns(undefined);
		oStub.withArgs(["sap/ui/mdc/field/FieldBaseDelegate"]).onFirstCall().callsFake((vDependencies, fnCallback, fnErrCallback) => {
			setTimeout(() => {
				oStub.wrappedMethod(vDependencies, fnCallback, fnErrCallback);
	...
	QUnit.test("FieldEditMode", async (assert) => {
		...
		aContent = oField.getAggregation("_content");
		assert.equal(oContent, aContent?.length > 0 && aContent[0], "Contont control not changed");
		assert.notOk(oContent.getEnabled(), "MultiInput is not enabled");
		assert.notOk(oField.getEditable(), "getEditable");

	});

	QUnit.test("display mode rendering, async loading of control", async (assert) => {

		const oStub = sinon.stub(sap.ui, "require");
		oStub.withArgs("sap/ui/mdc/field/TokenizerDisplay").onFirstCall().returns(undefined);
		oStub.withArgs(["sap/ui/mdc/field/TokenizerDisplay"]).onFirstCall().callsFake((vDependencies, fnCallback, fnErrCallback) => {
			setTimeout(() => {
				oStub.wrappedMethod(vDependencies, fnCallback, fnErrCallback);
			}, 0);
		});
	...
	QUnit.test("external control", async (assert) => {
		...
		const fnDone = assert.async();
		sap.ui.require(["sap/ui/model/odata/type/Single"], (aModules) => { // as type-module is loaded by creating control, check after this is done
			setTimeout(async () => { // as the order of the CallBack function ist not clear and a Promise is used inside LoadModules called in ContentFactory
	...
	QUnit.test("external display control", async (assert) => {
	...
	QUnit.test("external edit control", async (assert) => {
	...
	QUnit.test("external control via Delegate", async (assert) => {
		...
		const fnDone = assert.async();
		sap.ui.require(["sap/ui/model/odata/type/Single"], (aModules) => { // as type-module is loaded by creating control, check after this is done
			setTimeout(async () => { // as the order of the CallBack function ist not clear and a Promise is used inside Delegate
				let aContent = oField.getAggregation("_content");
				let oContent = aContent?.length > 0 && aContent[0];
				assert.ok(oContent, "Field has internal content");
	...
	QUnit.test("getFocusDomRef", async (assert) => {
	...
	QUnit.test("Label association", async (assert) => {
	...
	QUnit.test("Label property & connectLabel", (assert) => {
	...
	QUnit.test("Label property & default help", async (assert) => {
	...
	QUnit.test("enhanceAccessibilityState", async (assert) => {
	...
	QUnit.test("getAccessibilityInfo", async (assert) => {
	...
	QUnit.test("Empty Indicator", async (assert) => {
		...
		let aContent = oField.getAggregation("_content");
		let oContent = aContent?.length > 0 && aContent[0];
		let oContentDomRef = oContent?.getDomRef();
		assert.equal(oContent.getMetadata().getName(), "sap.ui.mdc.field.TokenizerDisplay", "sap.ui.mdc.field.TokenizerDisplay is used");
		...
		oContentDomRef = oContent?.getDomRef();
		assert.ok(oContentDomRef, "content control rendered");
		assert.equal(oDomRef?.children.length, 1, "Only one child rendered");
		assert.equal(oDomRef?.children[0], oContentDomRef, "Content control is child");

	});

	QUnit.test("FieldBaseDelegate.createContent", async (assert) => {
	...
	QUnit.test("getMaxConditionsForHelp", async (assert) => {
	...
	QUnit.test("getFormatOptions", async (assert) => {
	...
	QUnit.test("getFormFormattedValue with unit", async (assert) => {
	...
	QUnit.test("getFormFormattedValue with date", async (assert) => {
	...
	QUnit.test("getFormFormattedValue with showEmptyIndicator", async (assert) => {
	...
	QUnit.test("getFormObservingProperties", (assert) => {
	...
	QUnit.test("getFormRenderAsControl", async (assert) => {
		...
		if (!oContent.getFormRenderAsControl) { // control might not implent it
			oContent.getFormRenderAsControl = () => {};
		}
	...
	QUnit.test("value", (assert) => {
		...
		const fnDone = assert.async();
		setTimeout(() => { // to update ConditionModel
			let aContent = oFieldEditMulti.getAggregation("_content");
			let oContent = aContent?.length > 0 && aContent[0];
			assert.equal(oContent.getMetadata().getName(), "sap.ui.mdc.field.FieldMultiInput", "sap.ui.mdc.field.FieldMultiInput is used");
			assert.equal(oContent.getValue?.(), "", "no value set on MultiInput control");
			...
			assert.equal(oToken.getText?.(), "Test", "Text on token set");
			...
			oContent = aContent?.length > 0 && aContent[0];
			assert.equal(oContent.getMetadata().getName(), "sap.ui.mdc.field.TokenizerDisplay", "sap.ui.mdc.field.TokenizerDisplay is used");
			assert.equal(oContent.getTokens?.()[0].getText(), "Test", "Text set on Text control");
			...
			assert.equal(oContent.getValue?.(), "bar", "value set on Searchfield control");
	...
	QUnit.test("description", async (assert) => {
		...
		setTimeout(async () => { // to update ConditionModel
			...
			const aTokens = oContent.getTokens ? oContent.getTokens() : [];
			const oToken = aTokens[0];
			assert.equal(oToken.getText?.(), "Hello (Test)", "Text on token set");
			...
			aContent = oFieldDisplay.getAggregation("_content");
			oContent = aContent?.length > 0 && aContent[0];
			assert.equal(oContent.getTokens?.()[0].getText(), "Hello (Test)", "Text set on Text control");
	...
	QUnit.test("multipleLines", async (assert) => {
		...
		setTimeout(() => { // to update ConditionModel
			...
			assert.equal(oContent.getTokens?.()[0].getText(), "Test", "Text set on Token control");
			fnDone();
		}, 0);

	});

	QUnit.test("dataType Date", async (assert) => {
		...
		sinon.stub(oFieldEditSingle3, "getSupportedOperators").callsFake(() => {return [OperatorName.BT];});
		...
		const fnDone = assert.async();
		sap.ui.require(["sap/ui/model/type/Date", "sap/ui/model/odata/type/DateTime"], (aModules) => { // as type-module is loaded by creating control, check after this is done
			setTimeout(async () => { // to update ConditionModel
				await nextUIUpdate();
				let aContent = oFieldEditMulti.getAggregation("_content");
				let oContent = aContent?.length > 0 && aContent[0];
				let oToken = oContent?.getTokens()[0];
	...
	QUnit.test("dataType sap.ui.model.type.Time", async (assert) => {
	...
	QUnit.test("dataType DateTimeOffset", async (assert) => {
		...
		const fnDone = assert.async();
		sap.ui.require(["sap/ui/model/odata/type/DateTimeOffset"], (aModules) => { // as type-module is loaded by creating control, check after this is done
			setTimeout(async () => { // to update ConditionModel
				await nextUIUpdate();
				let aContent = oFieldEditSingle.getAggregation("_content");
				let oContent = aContent?.length > 0 && aContent[0];
	...
	QUnit.test("dataType Boolean", async (assert) => {
	...
	QUnit.test("dataType Boolean, load BoolValueHelp async", async (assert) => {
		...
		const fnDone = assert.async();
		sap.ui.require(["sap/ui/model/odata/type/Boolean"], (aModules) => { // as type-module is loaded by creating control, check after this is done
			setTimeout(async () => { // to update ConditionModel and wait for async control creation
				await nextUIUpdate();
	...
	QUnit.test("dataType sap.ui.model.type.Currency", async (assert) => {
		...
		setTimeout(() => { // to update ConditionModel
			...
			assert.equal(aTokens.length, 1, "MultiInput has one Token");
			const oToken = aTokens[0];
			assert.equal(oToken.getText?.(), sValue, "Text on token set");
			assert.equal(oContent2.getValue(), "USD", "Value set on currency-Input control");
	...
	QUnit.test("invalid dataType", async (assert) => {
	...
	QUnit.test("tooltip", async (assert) => {
	...
	QUnit.test("tooltip with external content", async (assert) => {
	...
	QUnit.test("ariaLabelledBy", async (assert) => {
	...
	QUnit.test("with multi value", async (assert) => {
		...
		const aContent = oField.getAggregation("_content");
		const oContent = aContent?.length > 0 && aContent[0];
		let oBindingInfo = oContent?.getBindingInfo("tokens");
		...
		oBindingInfo = oContent?.getBindingInfo("tokens");
		...
		oPromise.then((vResult) => {
			...
			assert.equal(oToken.getText?.(), "=X", "Text on token set");

			oSubmitPromise.then((vResult) => {
	...
	QUnit.test("with multi value and maxConditions", async (assert) => {
		...
		setTimeout(() => { // to update ConditionModel
			...
			assert.equal(oToken.getText?.(), "=X", "Text on token set");
			oToken = aTokens[1];
			assert.equal(oToken.getText?.(), "=Y", "Text on token set");
			fnDone();
		}, 0);

	});

	QUnit.test("wrong input on multi value", async (assert) => {
		...
		qutils.triggerKeydown(oContent.getFocusDomRef().id, KeyCodes.ENTER, false, false, false);
		setTimeout(async () => { // to wait for valueStateMessage in IE (otherwise it fails after control destroyed)
			assert.equal(iCount, 1, "change event fired once");
			assert.equal(iParseError, 0, "ParseError event not fired");
	...
	QUnit.test("wrong input on multi value without ConditionModel", async (assert) => {
	...
	QUnit.test("empty input on multi value with not nullable type", async (assert) => {
	...
	QUnit.test("with single value", async (assert) => {
	...
	QUnit.test("with single value and free condtitions", async (assert) => {
	...
	QUnit.test("with single value and dataType sap.ui.model.type.Currency", async (assert) => {
		...
		const aConditions = oCM.getConditions("Name");
		assert.equal(aConditions.length, 1, "one condition in Codition model");
		assert.ok(Array.isArray(aConditions[0]?.values), "condition value is array");
		assert.equal(aConditions[0]?.values[0][0], 1.11, "condition value0");
		assert.equal(aConditions[0]?.values[0][1], "EUR", "condition value1");
		assert.equal(aConditions[0]?.operator, OperatorName.EQ, "condition operator");

	});

	QUnit.test("with multi value and dataType sap.ui.model.type.Currency", async (assert) => {
		...
		assert.equal(aConditions.length, 1, "one condition in Codition model");
		assert.ok(Array.isArray(aConditions[0]?.values), "condition value is array");
		assert.equal(aConditions[0]?.values[0][0], 1.11, "condition value-number");
		assert.equal(aConditions[0]?.values[0][1], "EUR", "condition value-unit");
		assert.equal(aConditions[0]?.operator, OperatorName.EQ, "condition operator");
		...
		aConditions = oCM.getConditions("Name");
		assert.equal(aConditions.length, 2, "two conditions in Codition model");
		assert.ok(Array.isArray(aConditions[1]?.values), "condition value is array");
		assert.equal(aConditions[1]?.values[0][0], 1, "condition value0-number");
		assert.equal(aConditions[1]?.values[0][1], "EUR", "condition value0-unit");
		assert.equal(aConditions[1]?.values[1][0], 3, "condition value1-number");
		...
		aConditions = oCM.getConditions("Name");
		assert.equal(aConditions.length, 2, "two conditions in Codition model");
		assert.ok(Array.isArray(aConditions[0]?.values), "condition1 value is array");
		assert.equal(aConditions[0]?.values[0][0], 1.11, "condition1 value-number");
		assert.equal(aConditions[0]?.values[0][1], "USD", "condition1 value-unit");
		assert.equal(aConditions[0]?.operator, OperatorName.EQ, "condition1 operator");
		assert.ok(Array.isArray(aConditions[1]?.values), "condition2 value is array");
		assert.equal(aConditions[1]?.values[0][0], 1, "condition2 value0-number");
		assert.equal(aConditions[1]?.values[0][1], "USD", "condition2 value0-unit");
		assert.equal(aConditions[1]?.values[1][0], 3, "condition2 value1-number");
		assert.equal(aConditions[1]?.values[1][1], "USD", "condition2 value1-unit");
		assert.equal(aConditions[1]?.operator, OperatorName.BT, "condition2 operator");

	});

	QUnit.test("wrong input on single value", async (assert) => {
	...
	QUnit.test("with SearchField", async (assert) => {
		...
		assert.equal(aConditions.length, 1, "one condition in Codition model");
		assert.equal(aConditions[0]?.values[0], "X", "condition value");
		assert.equal(aConditions[0]?.operator, OperatorName.Contains, "condition operator");
		...
		assert.equal(aConditions.length, 1, "one condition in Codition model");
		assert.equal(aConditions[0]?.values[0], "Y", "condition value");
		assert.equal(aConditions[0]?.operator, OperatorName.Contains, "condition operator");
	...
	QUnit.test("with external content single value", async (assert) => {
	...
	QUnit.test("with external content multi value", async (assert) => {
	...
	QUnit.test("with type currency", async (assert) => {
	...
	QUnit.test("wrong input with type currency", async (assert) => {
		...
		const aContent = oField.getAggregation("_content");
		const oContent1 = aContent?.length > 0 && aContent[0];
		const oContent2 = aContent?.length > 0 && aContent[1];
		oContent1.focus();

		const fnDone = assert.async();
		jQuery(oContent1.getFocusDomRef()).val("A");
		qutils.triggerKeydown(oContent1.getFocusDomRef().id, KeyCodes.ENTER, false, false, false);
		setTimeout(() => { // to wait to update ValueState via binding
			assert.equal(iCount, 1, "change event fired once");
			assert.equal(sId, "F1", "change event fired on Field");
	...
	QUnit.test("pasting multiple values", async (assert) => {
		...
		oPromise.then((vResult) => {
			...
			let oToken = aTokens[0];
			assert.equal(oToken.getText?.(), "=AA", "Text on token set");
			oToken = aTokens[1];
			assert.equal(oToken.getText?.(), "=BB", "Text on token set");
			oToken = aTokens[2];
			assert.equal(oToken.getText?.(), "=CC", "Text on token set");
	...
	QUnit.test("pasting single value", async (assert) => {
	...
	QUnit.test("copy multiple values", async (assert) => {
	...
	QUnit.test("with external edit/display content", async (assert) => {
	...
	QUnit.test("value help enabled", async (assert) => {

		const oDummyIcon = new Icon("I1", { src: "sap-icon://sap-ui5", decorative: false, press: (oEvent) => {} }).placeAt("content");
		...
		assert.equal(oIcon?.getSrc(), "sap-icon://sap-ui5", "ValueHelpIcon set");
	...
	QUnit.test("with single value field", async (assert) => {
		...
		aConditions = oCM.getConditions("Name");
		assert.equal(aConditions.length, 1, "one condition in Codition model");
		assert.equal(aConditions[0].values[0], "Y", "condition value[0]");
		assert.equal(aConditions[0].values[1], "Navigate", "condition value[1]");
		assert.equal(aConditions[0].operator, OperatorName.EQ, "condition operator");
		assert.notOk(oField.hasPendingUserInput(), "no user interaction after ENTER");

		// navigate ti same condition again
		oValueHelp.fireNavigated({ condition: Condition.createItemCondition("Y", "Navigate") });
		qutils.triggerKeydown(oContent.getFocusDomRef().id, KeyCodes.ENTER, false, false, false);
		assert.ok(oValueHelp.onControlChange.calledOnce, "onControlChange called on ValueHelp");
		assert.notOk(oContent.getSelectedText(), "No text selected");
		...
		const oVHIcon = oContent?.getAggregation("_endIcon")[0];
		oVHIcon.firePress();
	...
	QUnit.test("remove value help", async (assert) => {
	...
	QUnit.test("Skip opening ValueHelp with pending content on focus loss", async (assert) => {
					...
					setTimeout(() => {
						...
						setTimeout(() => {
							...
							setTimeout(() => {
								...
								oField.setEditMode(FieldEditMode.Display);
								setTimeout(async () => { // to wait for promises that changes inner controls
									await nextUIUpdate();
	...
	QUnit.test("FocusInfo.targetInfo.silent - ValueHelp should not open on focus", (assert) => {
	...
	QUnit.test("value/key handling", async (assert) => {
		...
		assert.equal(aTokens.length, 1, "MultiInput has one Token");
		const oToken = aTokens[0];
		assert.equal(oToken.getText?.(), "I2", "Text on token is key, as ValueHelp has no description yet");
		...
		assert.equal(aConditions.length, 1, "Condition set on ValueHelp");
		assert.equal(aConditions[0]?.values[0], "I2", "selected item set as condition");
		...
		assert.equal(aConditions[0]?.values[1], "Item3", "condition description");
		assert.equal(aConditions[0]?.operator, OperatorName.EQ, "condition operator");
		...
		assert.equal(aConditions[0]?.values[1], "Item3", "condition description");
		assert.equal(aConditions[0]?.operator, OperatorName.EQ, "condition operator");
		assert.equal(aConditions[1]?.values[0], "I1", "condition value");
		assert.equal(aConditions[1]?.values[1], "Item1", "condition description");
		assert.equal(aConditions[1]?.operator, OperatorName.EQ, "condition operator");
		assert.notOk(oValueHelp.getItemForValue.called, "getItemForValue not called");
		assert.equal(oContent.getDOMValue(), "", "no value shown in inner control");
		assert.equal(oContent.getProperty("value"), "", "no value set in inner control"); // as getValue returns DomValue, not property
	...
	QUnit.test("select on single field", async (assert) => {
		...
		let aConditions = oCM.getConditions("Name");
		assert.equal(aConditions.length, 1, "one condition in Codition model");
		assert.equal(aConditions[0]?.values[0], "I2", "condition value");
		assert.equal(aConditions[0]?.values[1], "Item2", "condition description");
		assert.equal(aConditions[0]?.operator, OperatorName.EQ, "condition operator");
		assert.notOk(oValueHelp.getItemForValue.called, "getItemForValue not called");
		...
		assert.equal(aConditions.length, 1, "one condition in Codition model");
		assert.equal(aConditions[0]?.values[0], "I3", "condition value");
		assert.equal(aConditions[0]?.values[1], undefined, "condition description");
		assert.equal(aConditions[0]?.operator, OperatorName.EQ, "condition operator");
		const fnDone = assert.async();
		setTimeout(() => { // as text is updated async
	...
	QUnit.test("keyboard support on closed ValueHelp", (assert) => {
	...
	QUnit.test("keyboard support on closed ValueHelp with active navigation", (assert) => {
	...
	QUnit.test("keyboard support on open ValueHelp", (assert) => {
	...
	QUnit.test("navigation in open ValueHelp", (assert) => {
		...
		assert.equal(aConditions[1]?.values[1], "Item 3", "condition description");
		assert.equal(iSubmitCount, 0, "submit event not fired");
	...
	QUnit.test("navigation single Field in open ValueHelp", async (assert) => {
		...
		assert.equal(aConditions[0]?.values[0], "I3", "condition value");
		assert.equal(aConditions[0]?.values[1], "Item 3", "condition description");
		assert.equal(iSubmitCount, 0, "submit event not fired");
	...
	QUnit.test("navigation single Field in closed ValueHelp", async (assert) => {
	...
	QUnit.test("filtering and switching to value help", async (assert) => {
		...
		setTimeout(() => { // to wait for Promises and opening
			...
			setTimeout(() => { // to wait for Promises and opening and closing
				assert.equal(oValueHelp.getFilterValue(), "I", "FilterValue set");
				assert.equal(oValueHelp.getConditions().length, 1, "One condition set on ValueHelp");
				assert.ok(oValueHelp.open.called, "open called");
				...
				oValueHelp.close();
				setTimeout(() => { // to wait for closing
					fnDone();
				}, 400);
			}, 400);
		}, 400);
	...
	QUnit.test("change while open", (assert) => {
		...
		assert.equal(aConditions[1]?.values[0], "I1", "selected item set as condition");
		assert.ok(oValueHelp.close.called, "close called");
		aConditions = oCM.getConditions("Name");
		assert.equal(aConditions.length, 2, "two conditions in Codition model");
		assert.equal(aConditions[1]?.values[0], "I1", "condition value");
	...
	QUnit.test("invalid input", (assert) => {
		...
		setTimeout(() => { // to wait for update of valueState via Model
			...
			setTimeout(() => { // to wait for update of valueState via Model
				...
				assert.equal(aConditions.length, 2, "Condition set on ValueHelp");
				assert.equal(aConditions[1]?.values[0], "Unknown", "condition value");
				assert.ok(oValueHelp.close.called, "close called");
				aConditions = oCM.getConditions("Name");
				assert.equal(aConditions.length, 2, "two conditions in Codition model");
				assert.equal(aConditions[1]?.values[0], "Unknown", "condition value");
	...
	QUnit.test("invalid input on singleValue Field", async (assert) => {
		...
		setTimeout(() => { // to wait for valueStateMessage in IE (otherwise it fails after control destroyed)
			...
			oContent._$input.val("Unknown");
			qutils.triggerKeydown(oContent.getFocusDomRef().id, KeyCodes.ENTER, false, false, false);
			setTimeout(() => { // to update value state
				assert.equal(iCount, 1, "change event fired once");
				assert.equal(sId, "F1", "change event fired on Field");
				...
				let aConditions = oValueHelp.getConditions();
				assert.equal(aConditions.length, 1, "Condition set on ValueHelp");
				assert.equal(aConditions[0]?.values[0], "Unknown", "condition value");
				assert.ok(oValueHelp.close.called, "close called");
				aConditions = oCM.getConditions("Name");
				assert.equal(aConditions.length, 1, "two conditions in Codition model");
				assert.equal(aConditions[0]?.values[0], "Unknown", "condition value");
	...
	QUnit.test("one ValueHelp on 2 Fields", async (assert) => {
		...
		const oContent2 = aContent2?.length > 0 && aContent2[0];
		...
		assert.equal(aConditions[0]?.values[0], "I1", "condition value");
		...
		assert.equal(aConditions[1]?.values[0], "I2", "condition value");
		aConditions = oCM2.getConditions("Name");
		assert.equal(aConditions.length, 1, "one condition in Codition model of second Field");
		assert.equal(aConditions[0]?.values[0], "I1", "condition value");
	...
	QUnit.test("async parsing", (assert) => {
		...
		return oPromise.then((vResult) => {
			...
			assert.equal(oToken.getText?.(), "Item3", "Text on token set");
			assert.ok(oValueHelp.onControlChange.calledOnce, "onControlChange called on ValueHelp");
			assert.equal(oField._aAsyncChanges.length, 0, "no async changes stored in Field");

			return oSubmitPromise.then((vResult) => {
				assert.ok(vResult, "submit: Promise resolved");
				assert.deepEqual(vResult, aConditions, "submit: Promise result");
			}).catch((oException) => {
				assert.notOk(true, "submit: Promise must not be rejected");
	...
	QUnit.test("async parsing single value", async (assert) => {
	...
	QUnit.test("async parsing single value and same result", async (assert) => {
	...
	QUnit.test("invalid input with async parsing", (assert) => {
		...
		oPromise.then((vResult) => {
			...
			setTimeout(() => { // for model update
				setTimeout(() => { // for ManagedObjectModel update
					assert.equal(oContent.getValueState(), "None", "ValueState set");
					...
					assert.equal(aConditions[0]?.values[0], "I2", "condition value");
					assert.equal(aConditions[1]?.operator, OperatorName.EQ, "condition operator");
					assert.equal(aConditions[1]?.values[0], "Invalid", "condition value");
					assert.equal(aConditions[1]?.validated, ConditionValidated.NotValidated, "condition not validated");
					aConditions = oValueHelp.getConditions();
					...
					oPromise.then((vResult) => {
						...
						setTimeout(() => { // for model update
							setTimeout(() => { // for ManagedObjectModel update
								...
								assert.equal(aConditions[2]?.operator, OperatorName.Contains, "condition operator");
								assert.equal(aConditions[2]?.values[0], "Invalid", "condition value");
								assert.ok(oValueHelp.close.called, "close called");
								aConditions = oCM.getConditions("Name");
								assert.deepEqual(vResult, aConditions, "Promise result");
								...
								oPromise.then((vResult) => {
									...
									setTimeout(() => { // for model update
										setTimeout(() => { // for ManagedObjectModel update
											...
											assert.equal(aConditions[3]?.operator, OperatorName.EQ, "condition operator");
											assert.equal(aConditions[3]?.values[0], "Unknown", "condition value");
											assert.equal(aConditions[3]?.validated, ConditionValidated.NotValidated, "condition not validated");
											assert.ok(oValueHelp.close.called, "close called");
											aConditions = oCM.getConditions("Name");
											assert.deepEqual(vResult, aConditions, "Promise result");
	...
	QUnit.test("aria attributes on multi Field", async (assert) => {
		...
		assert.ok($FocusDomRef.attr("aria-describedby")?.search(sValueHelpEnabledID) >= 0, "ValueHelpEnabled text set");
		...
		assert.notOk($FocusDomRef.attr("aria-describedby")?.search(sValueHelpEnabledID) >= 0, "ValueHelpEnabled text not set");
	...
	QUnit.test("aria attributes on single Field", async (assert) => {
	...
	QUnit.test("aria attributes on single Field with only typeahed", async (assert) => {
	...
	QUnit.test("external control", async (assert) => {
		...
		setTimeout(() => { // to wait for Promises and opening
			setTimeout(() => { // second timeout because poipover opens async
				assert.ok(oValueHelp.open.calledWith(true), "oValueHelp opened as suggestion");
				assert.equal(oValueHelp.getFilterValue(), "I", "FilterValue set");

				oValueHelp.open.reset();
				oValueHelp.close();
				setTimeout(() => { // to wait for Promises and close
	...
	QUnit.test("Select currency", async (assert) => {
	...
	QUnit.test("Select in currency stand-alone", async (assert) => {
		...
		const oSuggestControl = oField.getControlForSuggestion();
		assert.equal(oSuggestControl?.getId(), oContent1?.getId(), "Currency control is used for suggestion");
	...
	QUnit.test("Enter currency with async parsing", async (assert) => {
		...
		// simulate select event to see if field is updated
		let fnResolve;
		let oFHPromise = new Promise((fResolve, fReject) => {
			fnResolve = fResolve;
		});
		const fnGetItemsForValue = (oConfig) => {
			vGetItemsForValue = oConfig.value;
			...
			setTimeout(() => { // for fieldGroup delay
				...
				oPromise.then((vResult) => {
					...
					setTimeout(() => { // update of Input
						...
						oFHPromise = new Promise((fResolve, fReject) => {
							fnResolve = fResolve;
						});
						...
						oPromise.then((vResult) => {
							...
							setTimeout(() => { // update of Input
								...
								sinon.stub(oConditionsType, "validateValue").callsFake((aConditions) => {
									if (aConditions?.[0]?.values[0][1] === "JPY") {
	...
	QUnit.test("navigate to currency", (assert) => {
	...
	QUnit.test("filtering for currency", async (assert) => {
	...
	QUnit.test("Select currency", (assert) => {
	...
	QUnit.test("displayed value", (assert) => {
	...
	QUnit.test("ValueHelp select", (assert) => {
	...
	QUnit.test("ValueHelp navigate", (assert) => {
	...
	QUnit.test("Rendering", (assert) => {
		...
		const fnDone = assert.async();
		setTimeout(() => { // to wait for promise
			let aContent = oField.getAggregation("_content");
			let oContent = aContent?.length > 0 && aContent[0];
			assert.ok(oContent, "content exist");
			assert.equal(oContent.getMetadata().getName(), "sap.m.Link", "sap.m.Link is used");
			assert.equal(oContent.getText?.(), "Test", "Text used");
			assert.notOk(oContent.getHref?.(), "no Href used");
			...
			setTimeout(async () => { // to wait for promise
				assert.equal(oContent.getHref?.(), "myHref", "Href used");
				assert.equal(oContent.getTarget?.(), "myTarget", "Target used");
	...
	QUnit.test("opening", (assert) => {

		const fnDone = assert.async();
		setTimeout(() => { // to wait for promise
			setTimeout(() => { // to wait rendering of Link
				const aContent = oField.getAggregation("_content");
				const oContent = aContent?.length > 0 && aContent[0];
				const oFieldInfo = Element.getElementById("F1-I");
	...
	QUnit.test("Clone", (assert) => {
		...
		setTimeout(async () => { // to wait for promise
			...
			setTimeout(async () => { // to wait for promise
				...
				let oCloneContent = aCloneContent?.length > 0 && aCloneContent[0];
				const oCloneFieldInfo = oClone.getFieldInfo();
				assert.ok(!!oCloneFieldInfo, "Clone has FieldInfo");
				assert.equal(oCloneFieldInfo?.getId(), "F1-I-myClone", "FieldInfo is cloned");
				assert.equal(oCloneContent.getMetadata().getName(), "sap.m.Link", "sap.m.Link is used on Clone");
				assert.equal(oCloneContent.getText?.(), "Test", "Text used on Clone");
				...
				setTimeout(() => { // to wait for promise
					...
					oCloneContent = aCloneContent?.length > 0 && aCloneContent[0];
	...
	const validateFieldGroup = (oEvent) => {
	...
	QUnit.module("FieldGroup handling", {
		beforeEach: async () => {
			...
			oIcon = new Icon("I3", { src: "sap-icon://sap-ui5", decorative: false, press: (oEvent) => {} }).placeAt("content"); // just dummy handler to make Icon focusable
			await nextUIUpdate();
		},
		afterEach() {
			oField.destroy();
	...
	QUnit.test("normal Field leave FieldGroup", (assert) => {
		...
		setTimeout(() => { // for fieldGroup delay
			...
			setTimeout(() => { // for fieldGroup delay
				...
				assert.equal(aFieldGroupIds?.length, 1, "One FieldGroupId");
				assert.equal(aFieldGroupIds?.[0], "MyFieldGroup", "FieldGroupId");
				fnDone();
			}, 0);
		}, 0);
	...
	QUnit.test("normal Field stay in FieldGroup", (assert) => {
	...
	QUnit.test("currency Field leave FieldGroup", (assert) => {
	...
	QUnit.test("currency Field stay in FieldGroup", (assert) => {

		const fnDone = assert.async();
		const aContent = oField2.getAggregation("_content");
		const oContent1 = aContent?.length > 1 && aContent[1];
		const oContent2 = aContent?.length > 1 && aContent[1];
		oContent2.focus();
		setTimeout(() => { // for fieldGroup delay
			oContent1.focus();
