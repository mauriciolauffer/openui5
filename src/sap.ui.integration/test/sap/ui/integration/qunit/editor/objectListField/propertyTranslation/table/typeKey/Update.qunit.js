				...
				EditorQunitUtils.isFieldReady(that.oEditor).then(function () {
					...
					EditorQunitUtils.isReady(that.oEditor).then(function () {
						...
						EditorQunitUtils.wait().then(function () {
							...
							assert.ok(deepEqual(EditorQunitUtils.cleanUUIDAndPosition(oNewObject), oDefaultNewObject), "SimpleForm field textArea: Has Default value");
							...
							EditorQunitUtils.wait().then(function () {
								...
								assert.ok(deepEqual(EditorQunitUtils.cleanUUIDAndPosition(oNewObject), Object.assign(deepClone(oDefaultNewObject, 500), {"key": "{i18n>string1}"})), "SimpleForm: Value updated");
				...
				EditorQunitUtils.isFieldReady(that.oEditor).then(function () {
					...
					EditorQunitUtils.isReady(that.oEditor).then(function () {
						...
						EditorQunitUtils.wait().then(function () {
							...
							EditorQunitUtils.wait().then(function () {
								assert.equal(oFormField.getValue(), "{i18n>string1}", "SimpleForm field 1: Has new value");
								assert.ok(deepEqual(EditorQunitUtils.cleanUUIDAndPosition(oTextArea.getValue()), Object.assign(deepClone(oDefaultNewObject, 500), {"key": "{i18n>string1}"})), "SimpleForm: Value updated");
								assert.ok(oFormField.getShowValueHelp(), "SimpleForm field 1: ShowValueHelp true");
								var oValueHelpIcon1 = oFormField._oValueHelpIcon;
								assert.ok(oValueHelpIcon1, "SimpleForm field 1: Value help icon exist");
								assert.ok(oValueHelpIcon1.getVisible(), "SimpleForm field 1: Value help icon visible");
										...
										EditorQunitUtils.wait().then(function () {
											...
											for (var i = 0; i < oLanguageItems1.length; i++) {
												...
												if (oCustomData && oCustomData.length > 0) {
													var sLanguage = oCustomData[0].getKey();
													var sExpectedValue = _oOriginExpectedValues["string1"][sLanguage] || _oOriginExpectedValues["string1"]["default"];
													var sExpectedValueState = "None";
													if (sLanguage === "en") {
														sExpectedValue = "string1 en";
														sExpectedValueState = "Information";
													}
													var sCurrentValue = oLanguageItems1[i].getContent()[0].getItems()[1].getValue();
													...
													var sValueState = oLanguageItems1[i].getContent()[0].getItems()[1].getValueState();
													assert.equal(sValueState, sExpectedValueState, "oTranslationPopover1 Content: item " + i + " " + sLanguage + ", value state: " + sValueState + ", expected: " + sExpectedValueState);
												}
											}
											var oNewObject = JSON.parse(oTextArea.getValue());
											assert.ok(deepEqual(EditorQunitUtils.cleanUUIDAndPosition(oNewObject), Object.assign(deepClone(oDefaultNewObject, 500), {"key": "{i18n>string1}"})), "SimpleForm: Value updated");
									...
									EditorQunitUtils.wait().then(function () {
										...
										EditorQunitUtils.wait().then(function () {
											...
											EditorQunitUtils.wait().then(function () {
												...
												EditorQunitUtils.wait().then(function () {
													oNewObject = JSON.parse(oTextArea.getValue());
													assert.ok(deepEqual(EditorQunitUtils.cleanUUIDAndPosition(oNewObject), Object.assign(deepClone(oDefaultNewObject, 500), {"key": "string value 2"})), "SimpleForm: Value updated");
									...
									EditorQunitUtils.wait().then(function () {
										...
										EditorQunitUtils.wait().then(function () {
											...
											EditorQunitUtils.wait().then(function () {
												...
												EditorQunitUtils.wait().then(function () {
													oNewObject = JSON.parse(oTextArea.getValue());
													assert.ok(deepEqual(EditorQunitUtils.cleanUUIDAndPosition(oNewObject), Object.assign(deepClone(oDefaultNewObject, 500), {"key": "{i18n>string2}"})), "SimpleForm: Value updated");
