...
function(
	...
	function applyExtensionPoint(oExtensionPoint, bSkipInsertContent) {
		...
		.then(ApplyProcessor.createDefaultContent.bind(
			...
			bSkipInsertContent,
			applyExtensionPoint,
			[]/* in base processor changes are not taken into account */)
		)
		.then(ApplyProcessor.addDefaultContentToExtensionPointInfo.bind(this, mExtensionPointInfo, bSkipInsertContent));
	}

	/**
	 * Implements the <code>Extension Points</code> provider by SAPUI5 flexibility that can be hooked in the
	 * <code>sap.ui.core.ExtensionPoint</code> life cycle. It is used only in design mode and does not consider
	 * the availability of UI changes. Therefore, the processor is not a precondition for applying internal flex changes.
	 *
