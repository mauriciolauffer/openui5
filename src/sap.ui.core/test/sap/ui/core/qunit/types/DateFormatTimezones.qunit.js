...
["ar", "he", "tr", "de", "en", "uk", "th", "zh_TW", "zh_CN"].forEach(function(sLocale) {
	...
	QUnit.test("parse of format symbol 'V': TimezoneUtils#getABAPTimezone is called", function (assert) {
		...
		assert.deepEqual(oResult, {timezone: "~sOtherABAPTimezone", length: "~sValue".length});
	});
	...
	QUnit.test("parse of format symbol 'V': given sTimezone is a time zone alias", function (assert) {
		...
		assert.deepEqual(oResult, {timezone: "~sABAPTimezone", length: "~sTimezoneTranslation".length});
	});
