...
sap.ui.define([
	...
	QUnit.test("set dialog state via API", function (assert) {
		...
		assert.strictEqual(Element.getElementById(this.oVSD.getSelectedSortItem()).getSelected(), true, "The selected sort item should have the selected flag set to true");
		...
		assert.strictEqual(Element.getElementById(this.oVSD.getSelectedGroupItem()).getSelected(), true, "The selected group item should have the selected flag set to true");
	...
	QUnit.test("The control is not invalidated on setting sort, group or filter Items", function (assert) {
		...
		this.oVSD.setSelectedFilterCompoundKeys(this.oFilterState);
...
/* The use of sinon clock requires isolated environment, otherwise it will interfere the async tests */
	...
	QUnit.test("Cancel on cancel button press event", function (assert){
		var done = assert.async(),
			...
			fnChecks = function () {
				...
				assert.strictEqual(Element.getElementById(this.getSelectedSortItem()).getSelected(), true, "The selected sort item should have the selected flag set to true");
				...
				assert.strictEqual(Element.getElementById(this.getSelectedGroupItem()).getSelected(), true, "The selected group item should have the selected flag set to true");
	...
	QUnit.test("Confirm event", function (assert){
		...
		this.oVSD.attachConfirm(function(oEvent) {
			...
			assert.deepEqual(that.oFilterState, oParams.filterCompoundKeys, "The event filter keys should have the same structure as the passed one");
	...
	QUnit.test("Grouping items selection", function (assert) {
		...
		assert.equal(Element.getElementById(sSelectedGroupItem), this.oVSD._oGroupingNoneItem, "GroupingNoneItem is selected when setSelectedGroupItem is called without params");
	...
	QUnit.test("Selectting Grouping None item by empty string", function (assert) {
		...
		//Check
		sSelectedGroupItem = this.oVSD.getSelectedGroupItem();
		assert.equal(Element.getElementById(sSelectedGroupItem), this.oVSD._oGroupingNoneItem, "GroupingNoneItem is selected when setSelectedGroupItem is called with empty string param");
	...
	QUnit.test("Check filter detail page and buttons render correctly [new]", function (assert) {
		var done = assert.async(),
				that = this;
		...
		setTimeout(function () {
			...
			setTimeout(function () {
				...
				assert.strictEqual(this.oVSD._getPage2().getCustomHeader().getContentMiddle()[0].getText(),
						Library.getResourceBundleFor("sap.m").getText("VIEWSETTINGS_TITLE_FILTERBY") + " Filter text B",
	...
	QUnit.test("setTitleAlignment test", function (assert) {
		...
		var oVSD = new ViewSettingsDialog(),
			...
			setTitleAlignmentSpy = this.spy(oVSD, "setTitleAlignment"),
			sInitialAlignment,
			sAlignment;
		...
		// initial titleAlignment test
		assert.ok(oVSD._getHeader().hasStyleClass(sAlignmentClass + sInitialAlignment),
