...
sap.ui.define([
	...
	QUnit.test("aria-describedby for value state with links contains announcement for the value state links shortcut", async function (assert) {
		...
		const sSingleLink = Library.getResourceBundleFor("sap.m").getText(sLinksTextId);
	...
	QUnit.test("aria-errormessage for value state with links not contains announcement for the value state links shortcut", async function (assert) {
		...
		const aAriaDescribedBy = oComboBox.getFocusDomRef().getAttribute("aria-describedby").split(" ");
		const bAriaDescribedByContainsAccForLinks = aAriaDescribedBy.some(function (sId) { return sId === oComboBox.getValueStateLinksShortcutsId();});
	...
	QUnit.test("Arrow down when the visible focus is on the input should move it to the first list item", async function (assert) {
		// Act
		this.oErrorComboBox.open();
		this.clock.tick(500);
		await nextUIUpdate(this.clock);
