...
sap.ui.define([
	...
	QUnit.module("sap.ui.fl.variants.VariantManagement", {
		beforeEach() {
			...
			sinon.stub(ManifestUtils, "getFlexReferenceForControl").returns(sFlexReference);
			...
			FlexState.addDirtyFlexObjects(sFlexReference, [
				FlexObjectFactory.createFlVariant({
					...
					variantManagementReference: "One",
					user: "A",
					layer: Layer.VENDOR
				}),
				FlexObjectFactory.createFlVariant({
			...
			this.oGetAppComponentForControlStub = sandbox.stub(Utils, "getAppComponentForControl").returns({
		...
		QUnit.test("Check getDesignMode", function(assert) {
			const oGetDesignModeStub = sandbox.stub(this._oVM, "getDesignMode").returns(true);
			assert.ok(this.oVariantManagement.getDesignMode(), "then design mode is true");

			oGetDesignModeStub.returns(false);
			assert.notOk(this.oVariantManagement.getDesignMode(), false, "then design mode is false");
		});
		...
		QUnit.test("Check title", async function(assert) {
			...
			assert.equal(this.oVariantManagement.getTitle().getText(), "Z2 Two");
		...
		QUnit.test("Check getVariants", async function(assert) {
			...
			assert.equal(aItems[1].getKey(), "v1");
		...
		QUnit.test("Check acc text", function(assert) {
			...
			assert.strictEqual(
				this._oVM.oVariantInvisibleText.getText(),
				this.oVariantManagement._oRb.getText("VARIANT_MANAGEMENT_SEL_VARIANT", ["Z2 Two"])
			);
		...
		QUnit.test("Check _checkVariantNameConstraints", function(assert) {
			...
			oInput.setValue("Z1 One");
			this._oVM._checkVariantNameConstraints(oInput, "v1");
			assert.equal(oInput.getValueState(), "None");

			this._oVM._checkVariantNameConstraints(oInput, "v2");
			assert.equal(oInput.getValueState(), "Error");
			oInput.destroy();
		});
		...
		QUnit.test("Create Variants List", async function(assert) {
			...
			assert.strictEqual(
				...
				this.oVariantManagement.getStandardVariantKey()
			);
			...
			// Add change on standard
			FlexState.addDirtyFlexObjects(
				sFlexReference,
			...
			// Switch variant
			const oSwitchVariantPromise = RtaQunitUtils.waitForMethodCall(sandbox, VariantManagerApply, "handleSelectVariant");
			this.oVariantManagement.setCurrentVariantKey("v4");
			await oSwitchVariantPromise;
			assert.notOk(this._oVM.oVariantSaveBtn.getVisible());
			assert.notOk(this.oVariantManagement.getModified());
			assert.ok(this._oVM.oVariantSaveAsBtn.getVisible());

			// Add change on custom variant
			FlexState.addDirtyFlexObjects(
				sFlexReference,
				[FlexObjectFactory.createUIChange({
					variantReference: "v4"
			...
			const oSwitchVariantPromise2 = RtaQunitUtils.waitForMethodCall(sandbox, VariantManagerApply, "handleSelectVariant");
			this.oVariantManagement.setCurrentVariantKey("v4");
			await oSwitchVariantPromise2;
			assert.notOk(this._oVM.oVariantSaveBtn.getVisible());
			assert.notOk(this.oVariantManagement.getModified());
		...
		QUnit.test("Current variant reference handling during switch with asynchronous steps (revert of changes)", async function(assert) {
			...
			.returns([
				FlexObjectFactory.createUIChange({
					variantReference: "One"
				})
			]); // no changes on other variants
			...
			// It is important that at this point in time the current variant reference still reflects the old variant,
			// because the asynchronous revert might take some time and in the meantime other code might query the current variant reference
			assert.strictEqual(
			...
			await oHandleSelectVariantPromise;
			assert.strictEqual(
				this.oVariantManagement.getCurrentVariantReference(),
				"v4",
		...
		QUnit.test("Create Variants List with non favorited Standard", function(assert) {
			...
			assert.equal(this._oVM.oVariantList.getItems()[0].getText(), "Z1 One");
		});
		...
		QUnit.test("Checking _handleVariantSaveAs", function(assert) {
			...
			this.oVariantManagement.attachEventOnce("save", function(oEvent) {
				...
				this.oVariantManagement.attachEventOnce("save", function() {
					assert.strictEqual(this._oVM.oInputName.getValueState(), "Error");
					fnDone();
				}.bind(this));
			...
			const aItems = this.oVariantManagement.getVariants();
			assert.ok(aItems);
			assert.equal(aItems.length, 5);
		...
		QUnit.test("Checking _handleManageCancelPressed", function(assert) {
			...
			var oItem = this._oVM._getItemByKey("v3");
			assert.ok(oItem);
			assert.equal(oItem.getTitle(), "Z3 Three");
		...
		QUnit.test("Checking _handleManageSavePressed; deleted item is NOT selected", async function(assert) {
			...
			this.oVariantManagement.attachManage(function(oEvent) {
				...
				const aDelItems = oData.deleted;
				assert.ok(oData.deleted);
				assert.equal(aDelItems.length, 2);
				assert.equal(aDelItems[0], "v1");
				assert.equal(aDelItems[1], "v4");

				const aRenamedItems = oData.renamed;
				assert.ok(aRenamedItems);
				assert.equal(aRenamedItems.length, 1);
				assert.equal(aRenamedItems[0].key, "v3");
			...
			const oItemRen = this._oVM._getItemByKey("v3");
			assert.ok(oItemRen);
			oItemRen.setTitle("New 3");

			const oItemDel = this._oVM._getItemByKey("v1");
			assert.ok(oItemDel);

			oItemDel.setTitle("New 1");

			this._oVM._handleManageDeletePressed(oItemDel);
			this._oVM._handleManageDeletePressed(this._oVM._getItemByKey("v4"));

			await this._oVM._handleManageSavePressed();
		...
		QUnit.test("Checking _handleManageSavePressed; deleted item is selected", async function(assert) {
			const fnDone = assert.async();
			sandbox.stub(FlexObjectManager, "saveFlexObjects").resolves();
			...
			this._oVM._handleManageDeletePressed(oItemDel);
			this._oVM._handleManageDeletePressed(this._oVM._getItemByKey("v2")); // will not be deleted, because different layer

			const oItemFav = this._oVM._getItemByKey("v4");
			assert.ok(oItemFav);
		...
		QUnit.test("Checking _handleManageDeletePressed; deleted item is default variant and Standard marked as non favorite", function(assert) {
			...
			assert.equal(this.oVariantManagement.getDefaultVariantKey(), "v1");
			var oItem = this._oVM._getItemByKey("One");
			assert.ok(oItem);
			assert.ok(!oItem.getFavorite());

			this._oVM._handleManageDeletePressed(this._oVM._getItemByKey("v1"));
		...
		QUnit.test("Check save in manage dialog with renaming", async function(assert) {
			const fnOnManage = sandbox.stub();
			sandbox.stub(FlexObjectManager, "saveFlexObjects").resolves();
			...
			await this._oVM._handleManageSavePressed();
			assert.ok(fnOnManage.calledOnce);
			...
			assert.ok(oInput);
			// Set to same value - invalid input
			assert.equal(oInput.getValue(), "Z2 Two");
			this._oVM._checkVariantNameConstraints(oInput, "v2");

			fnOnManage.resetHistory();
			await this._oVM._handleManageSavePressed();
			assert.ok(fnOnManage.notCalled);
		...
		QUnit.test("check getApplyAutomaticallyOnVariant method", function(assert) {
			...
			var oVariant = this.oVariantManagement._getInnerItemByKey("v1");
			assert.ok(oVariant);
	...
	QUnit.module("Given a variant management control in personalization mode", {
		async beforeEach() {
			...
			this.oView = await XMLView.create({
				id: `${sFlexReference}---mockview`,
				viewName: "fl.test.VariantManagementTestApp"
			});
			const MockComponent = UIComponent.extend("MockController", {
				metadata: {
					manifest: {
						"sap.app": {
							applicationVersion: {
				...
				createContent: () => {
					const oApp = new App(this.oView.createId("mockapp"));
					oApp.addPage(this.oView);
					return oApp;
				}
			});
		...
		afterEach() {
			...
			FlexState.clearRuntimeSteadyObjects(sFlexReference, sFlexReference);
			sandbox.restore();
		}
	}, function() {
		...
		function makeSelection(oVMControl, iIndex) {
			const oVariantListControl = oVMControl._getEmbeddedVM().oVariantPopOver.getContent()[0].getContent()[0];
			const oSelectedItem = oVariantListControl.getItems()[iIndex];
		...
		QUnit.test("when the control is switched to a new variant with no unsaved personalization changes", function(assert) {
			const fnDone = assert.async();
			const oCallListenerStub = sandbox.stub(this.oVMControl, "_executeAllVariantAppliedListeners");

			this.oUpdateCurrentVariantStub.onFirstCall().callsFake(async (mPropertyBag) => {
