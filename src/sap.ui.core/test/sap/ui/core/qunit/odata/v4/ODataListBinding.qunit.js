...
sap.ui.define([
	...
	QUnit.module("sap.ui.model.odata.v4.ODataListBinding", {
		beforeEach : function () {
			...
			this.oMetaModelMock = this.mock(this.oModel.oMetaModel);
			// by default, entity type is not found (and not needed)
			this.oMetaModelMock.expects("fetchObject").atLeast(0)
				.withExactArgs("/EMPLOYEES/").returns(SyncPromise.resolve());
...
[undefined, "AddVirtualContext"].forEach(function (sChangeReason) {
	...
	QUnit.test("initialize: resolved, refresh", function (assert) {
		...
		const oRefreshExpectation = this.mock(oBinding).expects("_fireRefresh")
...
[false, true].forEach(function (bAsync) {
	...
	QUnit.test(sTitle, function (assert) {
		...
		this.mock(oBinding).expects("resolveRefreshPromise")
			...
			.returns(oFetchContextsPromise);
		const oCreateExpectation = this.mock(oBinding).expects("createContextsForCachedData")
			.exactly(bAsync ? 1 : 0)
			.withExactArgs(5, 10);
...
[false, true].forEach(function (bAvailable, i) {
	...
	QUnit.test("getContexts: after refresh", function (assert) {
		...
		this.mock(oBinding).expects("fetchContexts")
			...
			.returns(oFetchContextsPromise);
		this.mock(oBinding).expects("createContextsForCachedData").never();
		this.mock(oBinding).expects("_fireChange")
...
[
	...
	QUnit.test(sTitle, function () {
		...
		oFetchContextsCall = this.mock(oBinding).expects("fetchContexts")
			.withExactArgs(0, 10, 100, undefined, false, sinon.match.func)
			.returns(oFetchContextsPromise);
		this.mock(oBinding).expects("createContextsForCachedData").withExactArgs(0, 10);
...
[0, 1].forEach(function (iCurrentEnd) {
	...
	//*********************************************************************************************
[false, true].forEach(function (bCreateOnly) {
	//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	[false, true].forEach(function (bCreated) {
		var sTitle = "createContexts, paging: less data than requested; create only : "
			+ bCreateOnly + ", w/ created: " + bCreated;
		...
		QUnit.test(sTitle, function (assert) {
			...
			assert.strictEqual(
				oBinding.createContexts(20 + iCreatedContexts, result(29, 20 + 29), bCreateOnly),
				true);

			assert.strictEqual(oBinding.bLengthFinal, !bCreateOnly,
				"some controls use bLengthFinal instead of isLengthFinal()");
			assert.strictEqual(oBinding.getLength(),
				49 + iCreatedContexts + (bCreateOnly ? 10 : 0));
			assert.strictEqual(oBinding.aContexts.length, 49 + iCreatedContexts);
			assert.strictEqual(oBinding.iMaxLength, bCreateOnly ? Infinity : 49);

			for (i = 37; i < 49; i += 1) {
				this.mock(oBinding.aContexts[i + iCreatedContexts]).expects("destroy")
					.exactly(bCreateOnly ? 0 : 1)
					.withExactArgs();
			}
			// code under test: delete obsolete contexts
			assert.strictEqual(
				oBinding.createContexts(20 + iCreatedContexts, result(17, 20 + 17), bCreateOnly),
				!bCreateOnly);

			assert.strictEqual(oBinding.isLengthFinal(), !bCreateOnly);
			assert.strictEqual(oBinding.getLength(),
				bCreateOnly ? 49 + 10 + iCreatedContexts : 37 + iCreatedContexts);
			assert.strictEqual(oBinding.aContexts.length,
				bCreateOnly ? 49 + iCreatedContexts : 37 + iCreatedContexts);
			assert.strictEqual(oBinding.iMaxLength, bCreateOnly ? Infinity : 37);

			// code under test
			assert.strictEqual(
				oBinding.createContexts(20 + iCreatedContexts, result(17), bCreateOnly),
				false,
...
//TODO cannot happen with our _Cache; _Cache doesn't read more than final length elements
			...
			assert.strictEqual(
				oBinding.createContexts(10000 + iCreatedContexts, result(0), bCreateOnly),
				false);
...
//TODO it can only shrink if iResultLength === 0
			...
			assert.strictEqual(
				oBinding.createContexts(50 + iCreatedContexts, result(0), bCreateOnly),
				!bCreateOnly);

			assert.strictEqual(oBinding.isLengthFinal(), !bCreateOnly);
			assert.strictEqual(oBinding.getLength(),
				50 + iCreatedContexts + (bCreateOnly ? 10 : 0));
			assert.strictEqual(oBinding.aContexts.length, 50 + iCreatedContexts);
			assert.strictEqual(oBinding.iMaxLength, bCreateOnly ? Infinity : 50);

			// code under test
			assert.strictEqual(
				oBinding.createContexts(30 + iCreatedContexts, result(0), bCreateOnly),
				!bCreateOnly);

			assert.strictEqual(oBinding.isLengthFinal(), !bCreateOnly);
			assert.strictEqual(oBinding.getLength(),
				bCreateOnly ? 50 + 10 + iCreatedContexts : 30 + iCreatedContexts);
			assert.strictEqual(oBinding.aContexts.length,
				bCreateOnly ? 50 + iCreatedContexts : 30 + iCreatedContexts);
			assert.strictEqual(oBinding.iMaxLength, bCreateOnly ? Infinity : 30);

			// code under test: preparation for following test for server-side paging: create a gap
			assert.strictEqual(
				oBinding.createContexts(100 + iCreatedContexts, result(20), bCreateOnly),
				true);
			...
			assert.strictEqual(
				oBinding.createContexts(0 + iCreatedContexts, aResults, bCreateOnly),
				true);
	...
	//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	let sTitle = `createContexts: reuse previous contexts, create only=${bCreateOnly}`;
	QUnit.test(sTitle, function (assert) {
		var oBinding = this.bindList("/EMPLOYEES", {/*oContext*/}),
		...
		this.mock(oBinding).expects("destroyPreviousContextsLater").exactly(bCreateOnly ? 0 : 1)
			.withExactArgs(["/EMPLOYEES/0"]);

		// code under test
		oBinding.createContexts(1, [{}, {}, {}], bCreateOnly);
	...
	//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	sTitle = `createContexts w/ keyPredicates, reuse previous contexts, create only=${bCreateOnly}`;
	QUnit.test(sTitle, function (assert) {
		var oBinding = this.bindList("/EMPLOYEES", {/*oContext*/}),
			oBindingMock = this.mock(oBinding),
			oContext0 = {},
	...
	//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	QUnit.test(`createContexts: shrink contexts, create only=${bCreateOnly}`, function (assert) {
		var oBinding = this.bindList("/EMPLOYEES", {}),
			oContext = {
				destroy : function () {}
			},
		...
		this.mock(oContext).expects("destroy").exactly(bCreateOnly ? 0 : 1).withExactArgs();

		// code under test
		oBinding.createContexts(1, aResults, bCreateOnly);

		assert.deepEqual(oBinding.aContexts, bCreateOnly ? [, , oContext] : []);
		assert.strictEqual(oBinding.bLengthFinal, !bCreateOnly);
		assert.strictEqual(oBinding.iMaxLength, bCreateOnly ? Infinity : 1);
	});

	//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	sTitle = `createContexts: do not reuse a created context, create only=${bCreateOnly}`;
	QUnit.test(sTitle, function (assert) {
		var oBinding = this.bindList("/EMPLOYEES"),
	...
	//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	sTitle = `createContexts: reuse a created context if kept alive, create only=${bCreateOnly}`;
	QUnit.test(sTitle, function (assert) {
		var oBinding = this.bindList("/EMPLOYEES"),
			oCreatedContext = Context.create(this.oModel, oBinding, "/EMPLOYEES('1')", -1,
				SyncPromise.resolve());
	...
	//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	sTitle = `createContexts: reuse a created context in a RH, create only=${bCreateOnly}`;
	QUnit.test(sTitle, function (assert) {
		const oBinding = this.bindList("/EMPLOYEES");
...
[false, true].forEach(function (bSame) {
	sTitle = `createContexts: reuse created from 'context' annotation, create only ${bCreateOnly},
		same : ${bSame}`;
	QUnit.test(sTitle, function (assert) {
		var oBinding = this.bindList("/EMPLOYEES"),
			oCreatedContext = Context.create(this.oModel, oBinding, "/EMPLOYEES('1')", -1,
...
[undefined, "~aElements~"].forEach(function (aElements) {
	[false, true].forEach(function (bCreatedAtEnd) {
		const sTitle = `createContextsForCachedData: ${aElements}, created at end=${bCreatedAtEnd}`;

	QUnit.test(sTitle, function () {
		const oBinding = this.bindList("/EMPLOYEES");
		const oCache = {
			getElements : mustBeMocked
		};
		oBinding.bFirstCreateAtEnd = bCreatedAtEnd;
		oBinding.iCreatedContexts = 7;
		const iOffset = bCreatedAtEnd ? 7 : 0;
		this.mock(oBinding).expects("withCache")
			.withExactArgs(sinon.match.func, "", true)
			.callsArgWith(0, oCache, "~path~");
		this.mock(oCache).expects("getElements")
			.withExactArgs("~path~", 23 + iOffset, 23 + 42 + iOffset)
			.returns(aElements);
		this.mock(oBinding).expects("createContexts").exactly(aElements ? 1 : 0)
			.withExactArgs(23 + iOffset, aElements, true);

		// code under test
		oBinding.createContextsForCachedData(23, 42);
	});
	});
});
	...
	[undefined, "('bar')"].forEach(function (sPredicate) {
		...
		QUnit.test(sTitle, function (assert) {
			...
			return oContext.created().then(function () {
				oBindingMock.expects("checkSuspended").withExactArgs(true);
				oBindingMock.expects("reset").withExactArgs(undefined, true);
				oBindingMock.expects("resetKeepAlive").withExactArgs();
...
[undefined, 42].forEach(function (iRank) {
	...
	QUnit.test("delete transient entity", function (assert) {
		...
		this.mock(oContext).expects("resetKeepAlive").withExactArgs();
...
[false, true].forEach((bHasPreviousData) => {
	...
	QUnit.test("fetchFilter: hierarchical filter", function (assert) {
		var oBinding = this.bindList("/Set"),
			...
			oMetaModelMock = this.oMetaModelMock,
			oPropertyMetadata = {$Type : "Edm.String"},
...
[FilterOperator.Any, FilterOperator.NotAny].forEach(function (sOperator) {
	QUnit.test("fetchFilter: " + sOperator + " - without predicate", function (assert) {
		...
		this.oMetaModelMock.expects("resolve").withExactArgs("p0", "~").returns("/resolved/path");
	...
	QUnit.skip("fetchFilter: filter with encoded path", function (assert) {
		...
		this.oMetaModelMock.expects("getMetaContext").withExactArgs(oBinding.sPath).returns("~");
		this.oMetaModelMock.expects("resolve").withExactArgs("AmountIn€", "~")
			.returns("/resolved/path");
		this.oMetaModelMock.expects("fetchObject").withExactArgs("/resolved/path")
			.returns(oPromise);
...
[{
	...
	[false, true].forEach((bDataAggregation) => {
		[
			{$Key : ["foo", "bar"]}, // all key properties are groups
			{$Key : ["foo", "bar", "baz"]}, // "baz" is not a group
			// Note: maybe metadata is wrong and not an entity type at all; in this case we do not
			// really care about the result, but true is OK
			{},
	...
	QUnit.test(sTitle, function (assert) {
		...
		this.mock(this.oModel).expects("resolve").withExactArgs("Set", sinon.match.same(oContext))
			.returns("~");
		this.oMetaModelMock.expects("resolve").withExactArgs("a", sinon.match.same(oMetaContext))
...
[false, true].forEach(function (bChanged) {
	...
	//*********************************************************************************************
[true, false].forEach((bSideEffectsRefresh) => {
	const sTitle = "doCreateCache w/ old cache, bSideEffectsRefresh = " + bSideEffectsRefresh;
...
["iCreatedContexts", "iDeletedContexts"].forEach(function (sProperty) {
	QUnit.test("doCreateCache w/ old cache, " + sProperty, function (assert) {
		...
		assert.strictEqual(
			// code under test
			oBinding.doCreateCache("resource/path", "~queryOptions~", "~context~",
				"deep/resource/path", "myGroup", /*bSideEffectsRefresh*/true, oOldCache),
			oOldCache);
...
[false, true].forEach(function (bWithOld) {
	...
	QUnit.test(sTitle, function (assert) {
		...
		assert.strictEqual(
			// code under test
			oBinding.doCreateCache("resource/path", "~queryOptions~", "~context~",
				"deep/resource/path", undefined, /*bSideEffectsRefresh*/false,
				bWithOld ? oOldCache : undefined),
	...
	[true, false].forEach(function (bOnRemoveCalled) {
		[true, false].forEach(function (bCreated) {
			...
			QUnit.test(sTitle, function (assert) {
				...
				oCacheRequestPromise = SyncPromise.resolve(Promise.resolve().then(function () {
					...
					if (!bOnRemoveCalled || bStillAlive) {
						oContextMock.expects("refreshDependentBindings")
							.withExactArgs("EMPLOYEES('2')", "groupId", false, undefined)
							.returns(oRefreshDependentsPromise);
					}
				}));
				...
				// code under test
				return oBinding.refreshSingle(oContext, "groupId", "~bLocked~", true)
				.then(function () {
...
[
	...
	QUnit.test("refreshSingle: bAllowRemoval && bWithMessages", function (assert) {
		...
		assert.throws(function () {
			// code under test
			oBinding.refreshSingle(oContext, /*sGroupId*/undefined, /*bLocked*/undefined,
				/*bAllowRemoval*/true, /*bKeepCacheOnError*/false, /*bWithMessages*/true);
...
[0, 3].forEach((iCount) => {
	...
	QUnit.test(sTitle, function (assert) {
		...
		this.mock(oBinding).expects("lockGroup").exactly(bAll && !bCountGiven ? 1 : 0)
			.withExactArgs()
			.returns("~oGroupLock~");
		this.mock(oBinding).expects("getKeepAlivePredicates").exactly(bCountGiven ? 0 : 1)
...
[{
	...
	QUnit.test(sTitle, function (assert) {
		...
		this.oMetaModelMock.expects("requestObject").withExactArgs("~meta~path~/")
			.callsFake(function () {
	...
	//*********************************************************************************************
	QUnit.test("getSelectionCount", function (assert) {
		const oBinding = this.bindList("/EMPLOYEES");
		const oBindingMock = this.mock(oBinding);

		oBindingMock.expects("getHeaderContext").withExactArgs().returns(oBinding.oHeaderContext);
		this.mock(oBinding.oHeaderContext).expects("getProperty").withExactArgs("$selectionCount")
			.returns(42);

		// code under test
		assert.strictEqual(oBinding.getSelectionCount(), 42);

		oBindingMock.expects("getHeaderContext").withExactArgs().returns(null);

		// code under test
		assert.strictEqual(oBinding.getSelectionCount(), undefined);
	});
...
[false, true].forEach(function (bAtEnd, i) {
	...
	QUnit.test("requestSelectedContexts: $$sharedRequest", function (assert) {
		...
		this.mock(oBinding.oHeaderContext).expects("isSelected").never();
		this.mock(oBinding).expects("hasPendingChanges").never();
	...
	QUnit.test("requestSelectedContexts: ask server", async function (assert) {
		...
		this.mock(oContextIn42).expects("isSelected").withExactArgs().returns(true);
		const oContextIn43 = {
			sPath : "/TEAMS('23')/TEAM_2_EMPLOYEES('43')",
			getPath : function () { return this.sPath; },
			isSelected : mustBeMocked
		};
		this.mock(oContextIn43).expects("isSelected").withExactArgs().returns(true);
		const oContextOut = {
			sPath : "/TEAMS('23')/TEAM_2_EMPLOYEES('n/a')",
			getPath : function () { return this.sPath; },
			isSelected : mustBeMocked
		};
		this.mock(oContextOut).expects("isSelected").withExactArgs().returns(true);
		this.mock(oBinding).expects("_getAllExistingContexts").withExactArgs()
			.returns([oContextIn42, oContextOut, oContextIn43]);
		...
		this.mock(oBinding.oCache).expects("requestFilteredOrderedPredicates")
			.withExactArgs(["('42')", "('n/a')", "('43')"], "~oGroupLock~")
			.resolves(["('43')", "('42')"]);
