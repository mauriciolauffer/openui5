sap.ui.define([
...
], function(Opa5, Arrangement, TestUtil, Action, Assertion, TestLibrary) {
	'use strict';
	...
	return function (opaTestOrSkip) {
		...
		opaTestOrSkip("When I Resize the columns and save it as new variant", function(Given, When, Then) {
		...
		opaTestOrSkip("When I close the 'View Settings' dialog without doing changes, the table has not been changed", function(Given, When, Then) {
			//check initially visible columns
			Then.iShouldSeeVisibleColumnsInOrder("sap.ui.mdc.table.Column", [
				"name", "foundingYear", "modifiedBy", "createdAt"
			]);
		...
		opaTestOrSkip("When I press on 'Sort' tab, sort p13n should show", function(Given, When, Then) {
			When.iPressOnButtonWithIcon(Arrangement.P13nDialog.Settings.Icon);
		...
		opaTestOrSkip("When I close the view settings dialog on 'sort' tab, the table has not been changed", function(Given, When, Then) {
		...
		// Define a new sorter
		// ----------------------------------------------------------------
		opaTestOrSkip("Add a sorter for 'Country'", function(Given, When, Then) {
			When.iPressOnButtonWithIcon(Arrangement.P13nDialog.Settings.Icon);
		...
		opaTestOrSkip("When I close the 'Selected Columns' button, the table has been changed", function(Given, When, Then) {
			When.iPressDialogOk();
		...
		// ----------------------------------------------------------------
		opaTestOrSkip("When I do some changes and press 'Cancel', the changes should be discarded", function(Given, When, Then) {
			When.iPressDialogOk();
			When.iPressOnButtonWithIcon(Arrangement.P13nDialog.Settings.Icon);
			Then.thePersonalizationDialogOpens();
		...
		// ----------------------------------------------------------------
		opaTestOrSkip("When I do some changes and press 'Escape', the changes should be discarded + Dialog should open again", function(Given, When, Then) {
			When.iPressDialogOk();
			When.iPressOnButtonWithIcon(Arrangement.P13nDialog.Settings.Icon);
			Then.thePersonalizationDialogOpens();
		...
		// ----------------------------------------------------------------
		opaTestOrSkip("When I select two additional columns and move them one up, the table should be changed", function(Given, When, Then) {
			When.iPressDialogOk();
			When.iPressOnButtonWithIcon(Arrangement.P13nDialog.Settings.Icon);
			Then.thePersonalizationDialogOpens();
		...
		// Close the dialog
		// ----------------------------------------------------------------
		opaTestOrSkip("Close the dialog", function(Given, When, Then) {
			When.iPressDialogOk();
		...
		// ----------------------------------------------------------------
		opaTestOrSkip("Reopen the dialog to see if it the items have been rearranged", function(Give, When, Then) {
			//Reopen the dialog
			When.iPressOnButtonWithIcon(Arrangement.P13nDialog.Settings.Icon);
			Then.thePersonalizationDialogOpens();
		...
		// ----------------------------------------------------------------
		opaTestOrSkip("check order", function(Given, When, Then) {
			//Reorder table items
			Then.iShouldSeeP13nItem("Created By", 0);
			Then.iShouldSeeP13nItem("Breakout Year", 1);
			Then.iShouldSeeP13nItem("Country", 2);
		...
		opaTestOrSkip("check search", function(Given, When, Then) {
		...
		opaTestOrSkip("check column header sort functionality: all previous sorters are deleted", function(Given, When, Then) {
			//close Dialog
			When.iPressDialogOk();
		...
		opaTestOrSkip("sort another column via context menu: only new column should be sorted", function(Given, When, Then) {
			//close Dialog
			When.iPressDialogOk();
		...
		opaTestOrSkip("Should be able to select all columns", function(Given, When, Then) {
			Given.iStartMyAppInAFrame({
				source: 'test-resources/sap/ui/mdc/qunit/p13n/OpaTests/appUnderTestTable/TableOpaApp.html',
				autoWait: true
			});
