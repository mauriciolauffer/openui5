...
sap.ui.define([
	...
	*/
	...
	QUnit.module("sap.ui.core.format.NumberFormat - currencies", {
		before() {
	...
	QUnit.test("Currency format default formatting preserveDecimals=true", function (assert) {
		var oFormat = NumberFormat.getCurrencyInstance({preserveDecimals:true});
	...
	QUnit.test("Currency format with showNumber and currency symbols", function (assert) {
		var oFormat = getCurrencyInstance({showNumber: false, currencyCode: false});
	...
	QUnit.test("Currency format with sMeasure and style", function (assert) {
		var oFormat = getCurrencyInstance({style: "long"});
		assert.strictEqual(oFormat.format(123456.789, "EUR"), "EUR" + "\xa0" + "123K", "123456.789 EUR");

		oFormat = getCurrencyInstance({style: "short"});
		assert.strictEqual(oFormat.format(123456.789, "EUR"), "EUR" + "\xa0" +  "123K", "123456.789 EUR");

		oFormat = getCurrencyInstance({style: "standard"});
		assert.strictEqual(oFormat.format(123456.789, "EUR"), "EUR" + "\xa0" +  "123,456.79", "123456.789 EUR");

		oFormat = getCurrencyInstance({style: "foo"});
		assert.strictEqual(oFormat.format(123456.789, "EUR"), "EUR" + "\xa0" +  "123,456.79", "123456.789 EUR");
	});
...
[// integrative tests for NumberFormat#getMaximumDecimals
	...
	QUnit.test("Currency formatOptions: take min of maxFractionDigits and decimals: " + i, function (assert) {
		...
		if (iDecimals === undefined) {
			...
			oNumberFormatMock.expects("getMaximumDecimals")
				.withExactArgs(sinon.match(oFormatOptions))
				.callThrough();
		} else {
			oNumberFormatMock.expects("getMaximumDecimals")
				.withExactArgs(sinon.match(oFormatOptions))
	...
	QUnit.test("format using currency instance", function (assert) {
		...
		sResult = oFormat.format(123456.789, "EUR"); // Empty string "";

		assert.strictEqual(sResult, "", "Empty string formatted.");
		...
		assert.strictEqual(sResult, "", "Empty string formatted.");
		assert.deepEqual(oFormat.parse(""), ["", undefined], "['', undefined] is returned.");
	});
	...
	QUnit.test("Currencies with numbers in their names - Log", function(assert) {
		this.oLogMock.expects("error").withExactArgs("Symbol '$' is defined multiple times in custom currencies.",
	...
	QUnit.test("Multiple custom currencies with same currency symbol", function(assert) {
		this.oLogMock.expects("error").withExactArgs("Symbol 'µ' is defined multiple times in custom currencies.",
			undefined, "NumberFormat");
...
[
	...
	QUnit.test("currency format/parse for currencies with letter 'K' in the measure symbol", function(assert) {
		//setup
		var oFormat = getCurrencyInstance({showMeasure: true});
	...
	QUnit.test("parse: custom currencies", function (assert) {
		...
		const oFormat = getCurrencyInstance({
				customCurrencies : {
					BTC : {digits : 2},
					CZK : {digits : 2},
					EUR : {digits : 2, symbol : "€"},
		...
		assert.deepEqual(oFormat.parse("2000 €"), [2000, "EUR"]);
		assert.deepEqual(oFormat.parse("€ 2000"), [2000, "EUR"]);

		this.oLogMock.expects("error")
			.withExactArgs("The parsed currency symbol '$' is defined multiple times in custom currencies."
				+ "Therefore the result is not distinct.").twice();

		assert.deepEqual(oFormat.parse("2000 $"), [2000, undefined], "Duplicate symbol is not parsed");
		assert.deepEqual(oFormat.parse("$ 2000"), [2000, undefined], "Duplicate symbol is not parsed");
		assert.deepEqual(oFormat.parse("‡ 2000"), null, "Unknown symbol is not parsed");
		assert.deepEqual(oFormat.parse("2000 euR"), [2000, "EUR"]);
		assert.deepEqual(oFormat.parse("2000 eu"), [2000, "EU"]);
		assert.deepEqual(oFormat.parse("2000 EU"), [2000, "EU"]);
		assert.deepEqual(oFormat.parse("2000 jPy"), [2000, "JPY"]);
		assert.deepEqual(oFormat.parse("czk 2000"), [2000, "CZK"]);
		assert.deepEqual(oFormat.parse("btc 2000"), [2000, "BTC"]);
		assert.deepEqual(oFormat.parse("2000 XYZ"), null, "No match found");
		assert.deepEqual(oFormat.parse("2000 dem"), [2000, "dem"]);
		assert.deepEqual(oFormat.parse("2000 DEM"), [2000, "DEM"]);
		assert.deepEqual(oFormat.parse("2000 DEM3"), [2000, "DEM3"]);
		assert.deepEqual(oFormat.parse("2000 dem3"), [2000, "DEM3"]);
		assert.deepEqual(oFormat.parse("2000 dEm"), null, "No clear match, 2 case insensitive matches");
		assert.deepEqual(oFormat.parse("2000 dEm3"), [2000, "DEM3"], "Duplicate was set to false");
		assert.deepEqual(oFormat.parse("2000 Dem4"), [2000, "Dem4"]);
		assert.deepEqual(oFormat.parse("2000 DEM4"), null);
		assert.deepEqual(oFormat.parse("ipß 2000"), [2000, "IPß"]);
		assert.deepEqual(oFormat.parse("ipss 2000"), [2000, "IPSS"]);
	});
	...
	//*********************************************************************************************
	// SNOW: DINC0239044
	// If no code list customizing is given (e.g. in case of CAP scenarios) a fallback to the CLDR configuration,
	// regarding the decimals, has to happen when formatting currencies.
...
[{
	...
	// The minimum of CLDRDecimals and maxFractionDigits is chosen as the number of decimals minFractionDigits
	// do not have an effect on the result (This specific case can occur but is considered to be a configuration issue)
	oFormatOptions: {maxFractionDigits: 0, minFractionDigits: 4},
	sExpectedResult: "USD\xa0100"
}, {
	// The decimals provided in the customCurrencies are selected to determine the minimum between decimals and
	// the maxFractionDigits which are then chosen as the number of decimal places for formatting
	oFormatOptions: {
	...
	// If customCurrencies are provided but do not define decimals for the currency, the minimum of the given decimals
	// and the maxFractionDigits is used for formatting
	oFormatOptions: {customCurrencies: {"USD": {}}, decimals: 4, maxFractionDigits: 10, minFractionDigits: 2},
	sExpectedResult: "USD\xa0100.0000"
}, {
	// If decimals is set explicitly to 0, no fallback to the CLDR must happen
	oFormatOptions: {decimals: 0},
	sExpectedResult: "USD\xa0100"
	...
	QUnit.test(`Currency type with no currency customizing and decimal options fall back to CLDR ${i}`,
			function (assert) {
		var oFormat = getCurrencyInstance(oFormatOptions);
	...
	//*********************************************************************************************
["short", "long"].forEach((sStyle) => {
	[
		// No fallback to the CLDR data in case "style: short/long" is set
		{sExpected: "USD\xa0123.457M", iMax: 3, iMin: 3, sStyle: sStyle},
		{sExpected: "USD\xa0123M", iMax: undefined, iMin: undefined,  sStyle: sStyle},
		{sExpected: "USD\xa0123M", iMax: 0, iMin: 0,  sStyle: sStyle},
		{sExpected: "USD\xa0123M", iMax: 0, iMin: undefined,  sStyle: sStyle},
		// No fallback to the CLDR data in case "style: short/long" is set, expecting all digits to be
		// displayed as we only define a minimum but no maximum for decimal places in the format options
		{sExpected: "USD\xa0123.456789M", iMax: undefined, iMin: 0,  sStyle: sStyle},
		// No usage of the customCurrencies decimals in case "style: short/long" is set
		{oCustomCurrencies: {USD: {decimals: 2}}, sExpected: "USD\xa0123.457M", iMax: 3, iMin: 3,  sStyle: sStyle},
		// Usage of the decimals as they are explicitly set in the format options
		{iDecimals: 2, sExpected: "USD\xa0123.46M", iMax: 3, iMin: 3,  sStyle: sStyle},
		{
			oCustomCurrencies: {USD: {decimals: 4}},
			iDecimals: 2,
			sExpected: "USD\xa0123.46M",
			iMax: 3,
			iMin: 3,
			sStyle: sStyle
		}
	].forEach(function({oCustomCurrencies, iDecimals, sExpected, iMax, iMin, sStyle}, i) {
		QUnit.test(`Currency format does not use customizing for decimals when style "${sStyle}" is set ${i}`,
				function (assert) {
			const oFormat = getCurrencyInstance({
					customCurrencies: oCustomCurrencies,
					decimals: iDecimals,
					maxFractionDigits: iMax,
					minFractionDigits: iMin,
					style: sStyle
				});

			// code under test
			assert.strictEqual(oFormat.format(["123456789", "USD"]), sExpected);
		});
	});
});
...
["short", "long"].forEach((sStyle) => {
	[
		// No fallback to the CLDR data in case "style: short/long" is set
		{sExpected: "USD\xa0123.456", iMax: 3, iMin: 3, sStyle: sStyle},
		{sExpected: "USD\xa0123", iMax: undefined, iMin: undefined,  sStyle: sStyle},
		{sExpected: "USD\xa0123", iMax: 0, iMin: 0,  sStyle: sStyle},
		{sExpected: "USD\xa0123", iMax: 0, iMin: undefined,  sStyle: sStyle},
		{sExpected: "USD\xa0123", iMax: undefined, iMin: 0,  sStyle: sStyle},
		// No usage of the customCurrencies decimals in case "style: short/long" is set
		{oCustomCurrencies: {USD: {decimals: 2}}, sExpected: "USD\xa0123.456", iMax: 3, iMin: 3,  sStyle: sStyle},
		// Usage of the decimals as they are explicitly set in the format options
		{iDecimals: 2, sExpected: "USD\xa0123.46", iMax: 3, iMin: 3,  sStyle: sStyle},
		{
			oCustomCurrencies: {USD: {decimals: 4}},
			iDecimals: 2,
			sExpected: "USD\xa0123.46",
			iMax: 3,
			iMin: 3,
			sStyle: sStyle
		}
	].forEach(function({oCustomCurrencies, iDecimals, sExpected, iMax, iMin, sStyle}, i) {
		const sTitle = `Currency format does not use customizing for decimals when style "${sStyle}"`
			+ ` even for small numbers ${i}`;
		QUnit.test(sTitle, function (assert) {
			const oFormat = getCurrencyInstance({
					customCurrencies: oCustomCurrencies,
					decimals: iDecimals,
			...
			// code under test
			assert.strictEqual(oFormat.format(["123.456", "USD"]), sExpected);
		});
	});
});
