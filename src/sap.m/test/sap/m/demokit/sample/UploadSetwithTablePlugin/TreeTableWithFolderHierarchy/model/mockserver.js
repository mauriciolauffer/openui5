sap.ui.define([
    ...
    return ManagedObject.extend("sap.m.sample.UploadSetwithTablePlugin.TreeTableWithFolderHierarchy.model.mockServer", {
        started: null,
        oModel: null,
        server: null,
        init: function () {
        },
        _updateModelWithData: function (oResponse, oExistingModelDataToUpdate, sMode, oFile, sUrl, oData, sCurrentPath) {
            switch (sMode) {

                case "Create":
                    // Get the current folder's items array
                    var aCurrentItems = this.oModel.getProperty(sCurrentPath || "/items");
                    if (aCurrentItems) {
                        var oNewItem = {
                            "documentId": oResponse && oResponse.id ? oResponse.id : uid(),
                            "fileName": oResponse.fileName,
                            "mimeType": oResponse.fileType,
                            "url": oResponse.fileUrl,
                            "size": oResponse.fileSize,
                        ...
                        for (var i = 0; i < aCurrentItems.length; i++) {
                            if (!aCurrentItems[i].isDirectory) {
                                aCurrentItems.splice(i, 0, oNewItem);
                                bInserted = true;
                                break;
                            }
                        }
                        if (!bInserted) {
                            aCurrentItems.push(oNewItem);
                        }

                        this.oModel.setProperty(sCurrentPath || "/items", aCurrentItems);
                    }
                    break;
                ...
                case "update":
                    if (oExistingModelDataToUpdate) {
                        ...
                        oExistingModelDataToUpdate.size = oFile && oFile.size ? oFile.size : 0;
                        oExistingModelDataToUpdate.mimeType = oFile && oFile.type ? oFile.type : "";
        ...
        start: function (sCurrentPath) {
            this.server = sinon.fakeServer.create();
            var that = this;
            this._sCurrentPath = sCurrentPath;
            ...
            this.server.respondWith("POST", RegExp("/uploadFiles", "i"), function (xhr) {
                ...
                if (data && data.items && iExistingDocumentId) {
                    oExistingfileObjectData = data.items.find(function (item) {
                        return item.documentId === iExistingDocumentId;
                    });
                }
                ...
                if (!oExistingfileObjectData) {
                    that._updateModelWithData(reponseObject, null, "Create", oFile, url, data, that._sCurrentPath);
                } else {
                    //this scenario is to simulate the upload of the empty document.
                    that._updateModelWithData(reponseObject, oExistingfileObjectData, "update", oFile, url, data, that._sCurrentPath);
                }
        ...
        updateExisitingDocument: function (oFileObject) {
            ...
            const oExistingfileObjectData = items.find(function (item) {
                return item.documentId === oFileObject.documentId;
            });
            if (oExistingfileObjectData) {
                oExistingfileObjectData.fileName = oFileObject.fileName;
                oExistingfileObjectData.url = oFileObject.url;
                oExistingfileObjectData.size = oFileObject.size;
                oExistingfileObjectData.mimeType = oFileObject.mimeType;
